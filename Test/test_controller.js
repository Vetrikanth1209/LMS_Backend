...
const Test = require('./test_schema');
...
router.post('/create', async (req, res) => {
  try {
    console.log('Request body:', req.body);
    let { test_name, test_language, status, test_id } = req.body;
...
// Get All Tests
router.get('/all_test', async (req, res) => {
    try {
...
router.get('/test_details/:test_id', async (req, res) => {
  try {
    ...
    const formattedDate = test.activeAt
      ? test.activeAt.toLocaleDateString('en-GB') // en-GB gives dd/MM/yyyy
      : null;

    res.status(200).json({
      test_name: test.test_name,
      test_language: test.test_language,
      test_total_score: test.test_total_score,
      activeAt: formattedDate
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.put('/update_test_module', async (req, res) => {
    try {
...
// Delete Test
router.delete('/delete_test_module/:test_id', async (req, res) => {
    try {
        const { test_id } = req.params;
