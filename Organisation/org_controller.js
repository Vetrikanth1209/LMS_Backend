...
const Organization = require("./org_schema");
...
router.put("/update_org_by_id/:org_id", async (req, res) => {
  try {
    const { org_id } = req.params;   // ✅ FIX
...
router.delete("/delete_org_by_id/:org_id", async (req, res) => {
  try {    const { org_id } = req.params;

    if (!org_id) {
      return res.status(400).json({ error: "Organization ID is required" });
    } 
    if (!/^[a-zA-Z0-9_-]+$/.test(org_id)) {
      return res.status(400).json({ error: "Invalid Organization ID format" });
    }
    const organization = await Organization.findOne({ mod_id: org_id });
    if (!organization) {
      return res.status(404).json({ error: "Organization not found" });
    } 
    await Organization.findOneAndDelete({ mod_id: org_id });
